# shared bootstrap for the desktop testbench: puts the fake hardware
# modules (tests/sim) on sys.path and grafts the micropython apis onto
# cpython's stdlib. imported for its side effects by conftest.py and
# provisioning_preview.py, and provides the boot helpers both use.
...
def purge_firmware_modules():
  # enviro/__init__.py runs board detection etc at import time, so each
  # boot re-imports the firmware with fresh state
...
def layout_device_fs(target_dir):
  ...
  (target_dir / "enviro").mkdir()
  shutil.copy(ROOT / "enviro" / "config_template.py", target_dir / "enviro" / "config_template.py")
  shutil.copytree(ROOT / "enviro" / "html", target_dir / "enviro" / "html")
